cc.Class({
    ...
    loadAction(viewList, length, i) {
        this.popView(viewList[i], true, function () {
            var closeIdx = i;
            i = i + 1;
            if (i < length) {
            ...
            this.popAnim(this.popSaver[viewList[closeIdx]], false, function () {
                //发出通知
                this.node.emit("loadPop", { pro: ((closeIdx + 1) / length) });
            }.bind(this));
        }.bind(this), 0.05);
    },
        ...
        if (view == null) {
            ...
            resManager.loadPrefab(name, function (prefab) {
                ...
                if (view != null) {
                    ...
                    this.popAnim(view, isOpen, cb, 0.05);
                }
            }.bind(this))
        } else {
            this.popAnim(view, isOpen, cb);
        }
    },

    //弹窗动画
    popAnim(view, isOpen, cb, time) {
        //整体结构对齐
        ...
        //缩回 
            ...
            Bg.runAction(
                cc.sequence(
                    cc.scaleTo(time, 0).easing(cc.easeElasticIn(5)),
                    cc.callFunc(function () {
                        view.active = false;
                        if (cb != null) {
                            cb(view);
