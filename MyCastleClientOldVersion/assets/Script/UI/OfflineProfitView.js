...
cc.Class({
    ...
    menuClick(event, type) {
        ...
        if (type == "receive") {
            ...
            gameApplication.onVideoBtnClick(function (isComplited) {
                if (isComplited) {
                    gameApplication.DataAnalytics.doEvent("offlineVideo");
    ...
    refreashVal(){
        this.money.string = gameApplication.countUnit(this.profit)[2];
    },
    ...
    showView(profit) {
        ...
        player.itemArrayAdd("pCurrency", 0, this.profit);
        this.getComponent(cc.Widget).unscheduleAllCallbacks();
    },
