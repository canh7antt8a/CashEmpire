cc.Class({
    ...
    menuClick(event, type) {
        ...
        if (type == "select") {
            ...
            if (player.itemArrayGet("pCurrency", mType) >= cost) {
                var endTime = player.itemArrayGet("gainPre", mType);
                //判断是否冷却
                if (endTime < curTime || endTime == 0 || endTime == null || endTime == undefined) {
                    curTime = curTime + this.selectInfo[0].infoList[idx].cdTime;
                    //设置冷却时间
                    player.itemArraySet("gainPre", mType, curTime);
                } else {
                    return;
                }
                player.itemArrayAdd("pCurrency", mType, -cost);
            ...
            if (player.itemArrayGet("pCurrency", 3) >= cost) {
                ...
                if (this.checkCDTime(idx)) {
                    this.surePop(function (isOk, isAD) {
                        if (isOk) {
                            if (isAD) {
                                //设置技能使用时间
                                var curTime = gameApplication.getCurTime();
    ...
    refreashVal() {
        ...
        for (var idx = 0; idx < length; idx = idx + 1) {
            if (idx < this.gainList.length) {
                var endTime = player.itemArrayGet("gainPre", idx);
                if (endTime != 0) {
                ...
                //价格
                if (this.selectInfo[0].infoList[idx].mType != 0) {
                    this.gainList[idx].price.string = gameApplication.countUnit(this.selectInfo[0].infoList[idx].cost)[3];
                } else {
                    this.gainList[idx].price.string = gameApplication.countUnit(this.selectInfo[0].infoList[idx].cost)[2];
                }
            }
    ...
    //刷新商品
    initView(cb) {
        resManager.loadConfig("PrestigeList", function (cof) {
    ...
    initSelectList(idx) {
        ...
        name1.dataID = this.selectInfo[idx].type;
        name2.dataID = this.selectInfo[idx].type;
    ...
    loadGainItem(idx) {
        ...
        //价格
        if (info.mType != 0) {
            price.string = gameApplication.countUnit(info.cost)[3];
        } else {
    ...
    //确认弹窗
    surePop(cb) {
        viewManager.popView("PrestigeVideoView", true, function (view) {
            var noBtn = cc.find("Bg/NoVideo", view);
            var okBtn = cc.find("Bg/Video", view);
            //加载描述
            var desc = cc.find("Bg/Desc", view).getComponent("LocalizedLabel");
            desc.dataID = "lang.prestigeUseDesc"
            //按钮事件绑定
            noBtn.off("click");
            noBtn.on("click", function () {
                viewManager.popView("PrestigeVideoView", false);
                if (cb != null) {
                    cb(true, false);
                }
            }.bind(this))
